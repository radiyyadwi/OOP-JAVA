...
public class Driver {
  ...
  public Driver(){
    ...
    LandHabitat b = new LandHabitat();
    WaterHabitat c = new WaterHabitat();
    Park d = new Park();
    Restaurant e = new Restaurant();
    Road f = new Road();
    Entrance g = new Entrance();
    Exit h = new Exit();
      ...
      for (int j = 0; j < width; j++) {
        ...
        if (msk.GetCC() == 'A' || msk.GetCC() == 'L' || msk.GetCC() == 'W') {
          ...
          if (num > 0) {  
            ...
            if ((tm == 'A' && cage_type[num] == "Air") || 
              (tm == 'L' && cage_type[num] == "Land") || 
              (tm == 'W' && cage_type[num] == "Water")) {
  ...
  public void DisplayVirtualZoo(Point upleft, Point downright){
    ...
    for (int i = 0;i < height; i++) {
      for (int j = 0;j<width;j++) {
        animal_matrix[i][j].SetX(-1);
        animal_matrix[i][j].SetY(-1);
      }
    }
  ...
  public void HitungMakanan(){
    ...
    for (int i = 0; i < max_cage; i++) {
      if (is_ada_cage[i]) {
        for (int j = 0; j < cage_list[i].NumberOfAnimal(); j++) {
          ...
          float foodnum = cage_list[i].GetAnimal(j).GetFoodNum();
          if (type == "Omnivore") {
            omni += foodnum;
          }
    ...
    System.out.println("%.2f", daging + " kg daging,");
    System.out.println("%.2f", sayur + " kg sayur, dan");
    System.out.println("%.2f", omni + " kg daging/sayur,");  
  }
    ...
    for (int i = 0;i < height; i++) {
      for (int j = 0;j < width; j++) {
        if (virtual_zoo.GetZooElmt(i,j).GetCellType() == "Facility") {
          ...
          entrance_list[n_entrance].SetX(j);
          entrance_list[n_entrance].SetY(i);
          n_entrance++;
          }
        }
      }
