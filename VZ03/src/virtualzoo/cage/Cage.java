...
public class Cage implements Renderable {
  ...
  */
  ...
  public void inputAnimal(Animal hewan, int areaId) {
    if (aman(hewan) && (0.3 * sizeArea - (jumlahHewan + 1) > 0) && (hewan.isTypeThere(cageType))) {
      ...
      for (int i = 0; i < jumlahHewan; i++) {
        if (hewan.isEnemy(listOfAnimal[i].getSpeciesName())) {
          ...
          System.out.println("Peletakan Animal: " + hewan.getSpeciesName() 
                              + " gagal, dapat membahayakan hewan dalam " +  "kandang");
        }
      }
    }
    if (safe) {
  ...
  public void moveAnimal() {
    ...
    for (int i = 0; i < jumlahHewan;i++) {
      ...
      if (dir == 4) {
        point.setY((listOfAnimal[i].getPosition()).getY() + 1);
        point.setX((listOfAnimal[i].getPosition()).getX());
      } else 
        if (dir == 1) {
          point.setY((listOfAnimal[i].getPosition()).getY() - 1);
          point.setX((listOfAnimal[i].getPosition()).getX());      
          ...
          if (dir == 2) {
            point.setY((listOfAnimal[i].getPosition()).getY());
            point.setX((listOfAnimal[i].getPosition()).getX() - 1);
          } else 
            if (dir == 3) {
              point.setY((listOfAnimal[i].getPosition()).getY());
              point.setX((listOfAnimal[i].getPosition()).getX() + 1);
      ...
      if (isPassAble(point)) {
        listOfAnimal[i].setPosition(point);
      }
    }
  }
